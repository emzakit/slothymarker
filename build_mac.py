...
ENTRY_POINT = 'main.py'
ICON_FILE = os.path.join('icons', 'app-icon.icns')
OUTPUT_DIR = 'dist'
...
def build():
    """Runs the PyInstaller build command for macOS."""
    if platform.system() != "Darwin":
        print("Error: This build script is intended for macOS only.")
        print("Please use build_win.py for Windows.")
        exit(1)

    print(f"\n--- Starting macOS build for {APP_NAME} ---")

    # The separator for --add-data on macOS is ':'
    separator = ':'
    ...
    command = [
        ...
        '--name', APP_NAME,
        # For macOS, --windowed automatically creates a .app bundle. We DO NOT use --onefile.
        '--windowed',
        '--icon', ICON_FILE,
    ]

    # Add data files. For .app bundles, '.' places them in the 'Resources' directory.
    ...
    try:
        ...
        if return_code == 0:
            print("\n--- Build Successful! ---")
            print(f"Application bundle created at: {os.path.join(OUTPUT_DIR, f'{APP_NAME}.app')}")
        else:
